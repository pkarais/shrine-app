    ...
    try {
      ...
      if (newBadges && newBadges.length > 0) {
        ...
        for (const award of newBadges) {
          ...
          await insertNotification(user.id, "Badge Earned!", `You earned "${badgeName}"!`, "badge_awarded")
        }
      }
      ...
      if (newDeductions && newDeductions.length > 0) {
        for (const d of newDeductions) {
          ...
          await insertNotification(user.id, "Points Deducted", d.reason || `-${d.points} points`, "points_deducted")
        }
      }
      ...
      if (newNominations && newNominations.length > 0) {
        ...
        for (const n of newNominations) {
          ...
          await insertNotification(user.id, "You've Been Nominated!", `You were nominated for "${badgeName}".`, "eom_nomination")
        }
      }
      ...
      if (leaderboard && leaderboard.length > 0) {
        ...
        if (isTopFive) {
          ...
          if (!hasFired(alertId)) {
            ...
            await insertNotification(user.id, "Top Five Leaderboard", `You're ranked #${myRank} on the leaderboard!`, "top_five")
          }
        }
        ...
        if (myRank !== null && previousRankRef.current && previousRankRef.current.dateKey === todayKey) {
          ...
          if (myRank < prevRank) {
            ...
            if (!hasFired(alertId)) {
              ...
              await insertNotification(user.id, "Leaderboard Jump!", `You moved up from #${prevRank} to #${myRank}!`, "leaderboard_jump")
            }
          }
        }

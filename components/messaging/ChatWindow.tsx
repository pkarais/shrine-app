...
export function ChatWindow({
  ...
  const loadMessages = useCallback(async () => {
    ...
    if (Array.isArray(data)) {
      ;(data as Message[]).forEach(m => seenIdsRef.current.add(m.id))
    }
    setLoading(false)
  ...
  return (
    <div className="flex flex-col h-full min-w-0">
      <div className="flex items-center gap-3 p-4 pl-5 border-b border-outline-variant/15">
        <button
          onClick={onBack}
      ...
      <form
        onSubmit={handleSend}
        className="flex items-end gap-2 p-3 px-5 border-t border-outline-variant/15"
      >
        <textarea
          ref={textareaRef}
          value={input}
          ...
          className="flex-1 input-surface px-4 py-2.5 text-sm resize-none overflow-hidden min-h-[40px] max-h-[120px] break-words"
          disabled={sending}
          rows={1}
        />
        <button
          ...
          className="w-10 h-10 rounded-full sacred-gradient flex items-center justify-center text-white disabled:opacity-40 transition-opacity active:scale-95 shrink-0"
        >

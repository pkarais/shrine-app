...
import { getSOPSignedUploadUrl, registerSOPUpload, addSOPExternalLink } from "@/lib/actions/sops"
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === "file") {
        ...
        // Step 1: Get a signed upload URL (fast server action, no file involved)
        setUploadProgress("Preparing upload...")
        const { signedUrl, filePath } = await getSOPSignedUploadUrl(file.name, category)

        // Step 2: Upload directly from browser to Supabase storage (bypasses Vercel)
        ...
        // Step 3: Register the DB record
        setUploadProgress("Saving record...")
        await registerSOPUpload({
          filePath,
          fileName: file.name,
          fileSize: file.size,
          title,
    ...
    <div className="bg-surface-container-low rounded-2xl p-6 space-y-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            ...
            {file ? (
              ...
              <>
                ...
                <p className="text-xs text-on-surface-variant/60 mt-1">PDF files only</p>
              </>
            )}
          </div>
        ) : (
        ...
        >
          ...
          {uploading ? (uploadProgress ?? "Saving...") : mode === "file" ? "Upload SOP" : "Save Link"}
        </button>
      </form>
    </div>
  )

...
import { getManagerAlerts, acknowledgeAlert, acknowledgeAllAlerts, deleteAcknowledgedAlerts } from "@/lib/actions/manager-alerts"
import { useAlertAudio } from "@/hooks/useAlertAudio"
import { Button } from "@/components/ui/Button"

const SEVERITY_CONFIG = {
  critical: { color: "bg-red-500", icon: AlertOctagon, bgColor: "bg-red-500/10 dark:bg-red-500/20", borderColor: "border-red-500/30" },
  warning:  { color: "bg-amber-500", icon: AlertTriangle, bgColor: "bg-amber-500/10 dark:bg-amber-500/20", borderColor: "border-amber-500/30" },
  info:     { color: "bg-blue-500", icon: Bell, bgColor: "bg-blue-500/10 dark:bg-blue-500/20", borderColor: "border-blue-500/30" }
}

const ALERT_AUDIO_MAP: Record<string, string> = {
  geofence_violation: "manager_geofence_alert",
  late_clock_in: "manager_late_alert",
  early_clock_in: "manager_late_alert",
        ...
        <div className="flex items-center gap-2">
          {unacknowledgedCount > 0 && (
            <button
              ...
              className="text-xs text-secondary hover:text-secondary/80 font-medium transition-colors disabled:opacity-50"
            >
              Mark All Read
            </button>
          )}

...
export function getShiftProgress(clockIn: Date, currentTime: Date) {
  // Clamp at 0 so server/client clock skew (clock_in slightly in the
  // future relative to browser now) can't render "-0.1h worked".
  const rawHours = (currentTime.getTime() - clockIn.getTime()) / (1000 * 60 * 60)
  const hoursWorked = Math.max(0, rawHours)
  const paidHours = Math.max(0, calculatePaidHours(clockIn, currentTime))

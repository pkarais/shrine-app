...
/**
 * Fan out a "new message" notification to every recipient so the in-app
 * NotificationBell + native browser notification + alert audio all fire
 * (the bell subscribes to postgres_changes on the notifications table).
 *
 * Best-effort: failures here MUST NOT block the message send. We log and
 * swallow so the user still sees their message persist.
 */
async function notifyRecipients(
  ...
  try {
    ...
    const body = trimmed.length > 140 ? trimmed.slice(0, 137) + "\u2026" : trimmed || "Sent you a message"
    const rows = uniqueRecipients.map((uid) => ({
      user_id: uid,
      title: `New message from ${senderName || "Shrine Ops"}`,
      body,
      type: "message",
    ...
    console.error("[notifyRecipients] failed to insert message notifications:", e)
  }
}

async function getSenderName(db: ReturnType<typeof createAdminClient>, userId: string): Promise<string> {
  try {
    const { data } = await db.from("profiles").select("full_name, email").eq("id", userId).maybeSingle()
    return (data?.full_name as string) || (data?.email as string) || "Shrine Ops"
  } catch {
    return "Shrine Ops"
  }
}
...
export async function sendMessage(recipientId: string, content: string, mediaUrls: string[] = []) {
  ...
  const senderName = await getSenderName(db, user.id)
  // reference_id = sender's user ID so the notification bell can deep-link
  // directly to the right conversation: /messages?user=<sender_id>
  await notifyRecipients(db, [recipientId], user.id, senderName, content, user.id)
...
export async function sendToManagers(content: string, mediaUrls: string[] = []) {
  ...
  if (error) throw new Error(error.message)
  // Notify every manager participant of the group (excluding the sender).
  try {
    const { data: members } = await db.from("conversation_participants").select("user_id").eq("conversation_id", convId)
    const recipientIds = (members || []).map((m: any) => m.user_id).filter(Boolean)
    const senderName = await getSenderName(db, user.id)
    // reference_id = sender's user ID for direct deep-link from notification bell
    await notifyRecipients(db, recipientIds, user.id, senderName, content, user.id)
  } catch (e) { console.error("[sendToManagers] notify failed:", e) }
...
export async function sendGroupMessage(conversationId: string, content: string, mediaUrls: string[] = []) {
  ...
  if (error) throw new Error(error.message)
  try {
    const { data: members } = await db.from("conversation_participants").select("user_id").eq("conversation_id", conversationId)
    const recipientIds = (members || []).map((m: any) => m.user_id).filter(Boolean)
    const senderName = await getSenderName(db, user.id)
    // reference_id = sender's user ID for deep-link; group recipients go to /messages?user=<sender>
    await notifyRecipients(db, recipientIds, user.id, senderName, content, user.id)
  } catch (e) { console.error("[sendGroupMessage] notify failed:", e) }
  return { success: true, message: data }
}
...
export async function getGroupMessages(conversationId: string, limit = 50) {
  ...
  const { data, error } = await db
    .from("group_messages")
    .select("id, sender_id, conversation_id, content, media_urls, created_at")
    .eq("conversation_id", conversationId)
    .order("created_at", { ascending: true })
    .limit(limit)
  if (error) throw new Error(error.message)

  // Batch-fetch profiles — no FK join (AGENTS.md convention)
...
export async function getMessagesWithUser(userId: string, limit = 50) {
  ...
  const { data, error } = await db
    .from("messages")
    .select("id, sender_id, recipient_id, content, media_urls, read_at, created_at")
...
export async function getContactsForUser() {
  ...
  if (myRole === "manager" || myRole === "admin") {
    ...
    roles = ["security", "manager", "council"]
  } else if (myRole === "council") {
    roles = ["operations", "security", "manager", "council"]
  } else {
    roles = ["operations", "manager", "council"]
  }

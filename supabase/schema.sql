...
CREATE POLICY "daily_brief_select_auth" ON daily_brief_issues FOR SELECT TO authenticated
  USING (true);
CREATE POLICY "daily_brief_insert_manager" ON daily_brief_issues FOR INSERT TO authenticated
  WITH CHECK (auth.jwt()->>'role' = 'manager');
CREATE POLICY "daily_brief_update_manager" ON daily_brief_issues FOR UPDATE TO authenticated
  USING (auth.jwt()->>'role' = 'manager');

CREATE POLICY "daily_brief_sections_select_auth" ON daily_brief_sections FOR SELECT TO authenticated
  USING (true);
CREATE POLICY "daily_brief_sections_insert_manager" ON daily_brief_sections FOR INSERT TO authenticated
  WITH CHECK (auth.jwt()->>'role' = 'manager');
CREATE POLICY "daily_brief_sections_update_manager" ON daily_brief_sections FOR UPDATE TO authenticated
  USING (auth.jwt()->>'role' = 'manager');

CREATE OR REPLACE VIEW v_daily_brief_archive AS
SELECT
  i.id,
  i.brief_date,
  i.title,

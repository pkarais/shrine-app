...
const MANAGER_ALERTS: AlertCategory[] = [
  {
    ...
    alerts: [
      ...
      { id: "manager_late", label: "Late Clock-In", description: "Staff member clocked in past grace period", audioKey: "manager_late_alert", defaultEnabled: true },
      { id: "manager_early", label: "Early Clock-In", description: "Staff clocked in 30+ min before scheduled start", audioKey: "manager_late_alert", defaultEnabled: true },
  ...
  return (
    <>
      ...
      <div className="card-surface rounded-2xl p-6 mb-6 border border-outline-variant/30">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="flex items-center justify-between p-4 rounded-xl bg-surface-container">
            ...
            <Switch checked={globalAudioEnabled} onCheckedChange={(v) => { setGlobalAudioEnabled(v); localStorage.setItem("shrine-alert-settings", JSON.stringify({ alerts: alertSettings, globalAudio: v, globalText: globalTextEnabled, lastUpdated: new Date().toISOString() })) }} />
          </div>
          <div className="flex items-center justify-between p-4 rounded-xl bg-surface-container">
            <div className="flex items-center gap-3">
            ...
            <Switch checked={globalTextEnabled} onCheckedChange={(v) => { setGlobalTextEnabled(v); localStorage.setItem("shrine-alert-settings", JSON.stringify({ alerts: alertSettings, globalAudio: globalAudioEnabled, globalText: v, lastUpdated: new Date().toISOString() })) }} />
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 flex justify-end">
        <Button onClick={() => { localStorage.setItem("shrine-alert-settings", JSON.stringify({ alerts: alertSettings, globalAudio: globalAudioEnabled, globalText: globalTextEnabled, lastUpdated: new Date().toISOString() })) }} variant="primary">
          Save Changes
        </Button>
      </div>
    </>

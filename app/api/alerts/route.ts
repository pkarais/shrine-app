...
export async function GET() {
  try {
    ...
    // Single batch query for all assignments — no N+1
    const eventIds = (events || []).map((e: any) => e.id)
    const assignmentsByEvent = new Map<number, { role_assigned: string }[]>()
    ...
    for (const event of events || []) {
      ...
      eventAssignments.forEach((a: any) => { if (counts[a.role_assigned] !== undefined) counts[a.role_assigned]++ })

...
import { StaffOnlineNowCard } from "@/components/manager/StaffOnlineNowCard"
...
import { ArchiveControls } from "@/components/manager/ArchiveControls"
import { getArchiveStats } from "@/lib/actions/archive"
import VisitorTotalsCard from "@/components/manager/VisitorTotalsCard"
import { getVisitorTotals } from "@/lib/actions/visitor-rollups"
import EventCoverageGapsCard from "@/components/manager/EventCoverageGapsCard"
import { getEventCoverageGaps, syncEventCoverageAlerts } from "@/lib/actions/event-coverage"
import { TopAppBar } from "@/components/layout/TopAppBar"
import { LABOR } from "@/constants"
import { IncidentReportViewer } from "@/components/manager/IncidentReportViewer"
...
export default async function ManagerPage() {
  ...
  const overtimeExcessHours = overtimeAlerts.reduce((sum: number, shift: any) => sum + Math.max(0, shift.paidHours - LABOR.OVERTIME_THRESHOLD_HOURS), 0)
  ...
  const optimizerSuggestions = overtimeAlerts.slice(0, 8).map((alert: any) => ({
    ...
    excessHours: Math.max(0, alert.paidHours - LABOR.OVERTIME_THRESHOLD_HOURS),
  }))

  // Archive stats (manager-only) — used by the Archive & Clear card.
  let archiveStats
  try {
    archiveStats = await getArchiveStats()
  } catch {
    archiveStats = {
      live: { messages: 0, group_messages: 0, incidents: 0 },
      archived: { messages: 0, group_messages: 0, incidents: 0 },
      last_run: null,
    }
  }

  // Visitor totals (today by default; user can flip to week/biweek/month).
  let visitorTotalsToday = null as any
  try {
    visitorTotalsToday = await getVisitorTotals("today")
  } catch {
    visitorTotalsToday = null
  }

  // Per-event coverage gaps (events whose required roles are not covered
  // by the live schedule — snapshot + grid edits). This is the single
  // authoritative gap surface for the Command Center.
  let eventCoverageGaps: any[] = []
  try {
    eventCoverageGaps = await getEventCoverageGaps(14)
  } catch {
    eventCoverageGaps = []
  }

  // Mirror uncovered events into manager_alerts so the Alerts feed and
  // the Red Flags surface reflect them. Auto-clears alerts for events
  // that have since been covered.
  try {
    await syncEventCoverageAlerts(eventCoverageGaps)
  } catch (err) {
    console.error("syncEventCoverageAlerts failed:", err)
  }
    ...
    <>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
        ...
        <div className="lg:col-span-8">
          <div className="h-full rounded-2xl overflow-hidden shadow-lg border border-outline-variant/20 relative">
            <Image
            ...
            <div className="relative z-10 h-full flex flex-col justify-end p-8 space-y-6">
              <div className="w-fit rounded-2xl bg-black/50 backdrop-blur-md p-4 border border-white/10 shadow-lg space-y-1">
                <Image
                ...
                />
                <p className="font-label text-xs uppercase tracking-[0.05em] text-white/70">Sacred Blueprint / Operations</p>
                <h2 className="text-display-lg text-white">Manager&apos;s Command Center</h2>
              </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <ArchiveControls initialStats={archiveStats} />
        <VisitorTotalsCard initial={visitorTotalsToday} />
        <EventCoverageGapsCard initialGaps={eventCoverageGaps} />
      ...
      <section className="bg-surface-container-low rounded-2xl p-8">
        ...
        <IncidentReportViewer incidents={managerIncidents} />
      </section>
      ...
      {/* Staff Media + Direct Comms + Online Now */}
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <MediaFolders />
        </div>
        <div className="lg:col-span-1 space-y-6">
          <StaffOnlineNowCard staff={(profiles ?? []) as any} />

...
export default async function ProfilePage() {
  ...
  if (isStaff) {
    ...
    // No FK join — batch-fetch event titles separately (AGENTS.md convention)
    ...
    const rawShifts = shifts || []
    const eventIds = Array.from(new Set(rawShifts.map((s: any) => s.event_id).filter(Boolean)))
    ...
    if (eventIds.length > 0) {
      const { data: eventRows } = await supabase
        ...
        .select("id, title")
        .in("id", eventIds)
      for (const e of eventRows || []) eventTitleMap.set(e.id, e.title)
    }
    completedShifts = rawShifts.map((s: any) => ({
      ...s,
      events: s.event_id ? { title: eventTitleMap.get(s.event_id) ?? null } : null,
    }))
  }

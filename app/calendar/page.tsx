...
import { getEventCoverageForRange } from "@/lib/actions/event-coverage"
...
export default async function CalendarPage({
  ...
  // Cross-reference the day's scheduled staff into event assignments so
  // events show as covered. Source of truth is the latest uploaded
  // schedule (PDF / Excel / CSV / paste) — falls back to the hardcoded
  // static data only if no upload has ever been saved.
  const templateForDay = await getTemplateScheduleForRange(selectedDateStr, selectedDateStr)
  const dailySchedule = templateForDay.shiftsByDate[selectedDateStr] || []
  const SCHEDULE_ROLE_MAP: Record<string, string> = {
    // Static fallback map (used when the snapshot's staffRoleMap is empty).
  ...
  // Snapshot-derived staff-to-role labels override the fallback map.
  for (const [staffName, label] of Object.entries(templateForDay.staffRoleMap)) {
    const lower = label.toLowerCase()
    // Map snapshot's Director/Porter/Greeter/Security to event-assignment roles
    if (lower === "director") SCHEDULE_ROLE_MAP[staffName] = "director"
    else if (lower === "porter" || lower === "operations") SCHEDULE_ROLE_MAP[staffName] = "operations"
    else if (lower === "greeter") SCHEDULE_ROLE_MAP[staffName] = "greeter"
    else if (lower === "security") SCHEDULE_ROLE_MAP[staffName] = "security"
  }
  ...
  // Per-event coverage (schedule overlap + add-on assignments). Used by
  // the timeline to render authoritative Covered / Coverage gap badges.
  // Failures fall back to assignment-count badges so the page still renders.
  let coverageByEvent: Record<number, CoverageResult> = {}
  try {
    const rows = await getEventCoverageForRange(selectedDateStr, selectedDateStr)
    coverageByEvent = Object.fromEntries(rows.map((r) => [r.eventId, r.coverage]))
  } catch {
    coverageByEvent = {}
  }
  ...
  // Snapshot-derived weekly template for the RecurringScheduleCalendar so
  // it no longer reads from the hardcoded static schedule.
  const weeklyTemplate = await getTemplateScheduleForRange(weekStartStr, weekEndStr)
      ...
      <section className="mb-12">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <div className="flex flex-col items-end gap-3">
            ...
            events={(events || []).filter((event) => !String(event.title || "").startsWith("Regular Shrine Open ")).map((event) => ({ id: event.id, title: event.title, start_time: event.start_time, end_time: event.end_time || null }))}
            staff={staffOptions}
            ...
            <div className="space-y-4">
              {events && events.filter(e => e.category === 'major_feast').map((e, idx) => (
                <div key={idx} className="p-4 bg-tertiary-container rounded-xl text-white">
                   ...
                   <h4 className="font-bold text-sm">{String(e.title || "").replace(/ \d{4}-\d{2}-\d{2}$/, "")}</h4>
                ...
                {weekDays.map((day, i) => {
                  ...
                  return (
                    <a
                      key={i}
                      href={dayHref}
                      className={`min-h-[76px] p-3 sm:p-4 text-center transition-colors hover:bg-surface-container-highest ${i < 6 ? "border-r border-outline-variant/15" : ""} ${isSelected ? "bg-primary text-on-primary" : ""}`}
                    >
              ...
              <div className="space-y-4">
                {filteredEvents && filteredEvents.length > 0 ? (
                  <CalendarEventTimeline
                    ...
                    coverageByEvent={coverageByEvent}
                  />
                ) : (
      ...
      {profileRole === "manager" && (
        <section className="mt-12">
          <RecurringScheduleCalendar
            ...
            templateShiftsByDate={weeklyTemplate.shiftsByDate}
            templateStaffRoles={weeklyTemplate.staffRoleMap}
            templateSource={weeklyTemplate.source}
            canEdit={true}
          />
        </section>
      )}

...
import { ArrowLeftCircle, ArrowRightCircle, CheckCircle2, ExternalLink, FileDown, FilePlus2, Loader2, Mail, Newspaper, Pencil, Save, XCircle } from "lucide-react"
...
import { easternYearMonth } from "@/lib/eastern-time"
...
export default function OperationsBriefGeneratorPage() {
  ...
  const [issueMonth, setIssueMonth] = useState(() => easternYearMonth())
  ...
  const [editingSopSpotlight, setEditingSopSpotlight] = useState(false)
  const [sopSpotlightTitle, setSopSpotlightTitle] = useState("")
  const [sopSpotlightBody, setSopSpotlightBody] = useState("")
  ...
  async function handleGenerateDraft() {
    try {
      ...
      setUrlInput(result.issue.website_url || "")
      setPdfUrl(result.issue.pdf_url || "")
      const sopSection = result.sections.find((s: OperationsBriefSection) => s.section_key === "sop_spotlight")
      setSopSpotlightTitle(sopSection?.content?.sop_title ?? "")
      setSopSpotlightBody(sopSection?.markdown_body ?? "")
  ...
  async function handleSaveSopSpotlight() {
    const sopSection = sections.find((s) => s.section_key === "sop_spotlight")
    if (!sopSection?.id) return
    try {
      setLoading(true)
      const { updateBriefSection } = await import("@/lib/actions/operations-brief")
      await updateBriefSection(
        sopSection.id,
        sopSpotlightBody,
        sopSpotlightTitle ? { sop_title: sopSpotlightTitle } : undefined
      )
      setSections((prev) =>
        prev.map((s) =>
          s.section_key === "sop_spotlight"
            ? { ...s, markdown_body: sopSpotlightBody, content: { ...s.content, sop_title: sopSpotlightTitle } }
            : s
        )
      )
      setEditingSopSpotlight(false)
      setMessage("SOP Spotlight updated.")
    } catch (err: any) {
      setError(err.message ?? "Unable to save SOP spotlight.")
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleSaveUrl() {
    ...
    try {
      setLoading(true)
      setError(null)
      const { saveWebsiteUrl } = await import("@/lib/actions/operations-brief")
      await saveWebsiteUrl(issue.id, urlInput)
      setWebsiteUrl(urlInput)
      setIssue({ ...issue, website_url: urlInput || null })
      setMessage("Website URL saved.")
    } catch (err: any) {
      setError(err.message ?? "Unable to save URL.")
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleSendEmail() {
    ...
    try {
      ...
      setError(null)
      const { sendOperationsBriefEmail } = await import("@/lib/actions/operations-brief")
      await sendOperationsBriefEmail(issue.id, emailRecipients)
        ...
        <div className="mx-auto max-w-7xl">
          ...
          >
            ...
            <div className="relative z-10 p-6 md:p-8 w-full">
              <Image
                ...
                width={176}
                height={70}
                className="w-36 sm:w-44 h-auto object-contain mb-4"
              />
          ...
          <div className="mb-6 flex flex-wrap items-end gap-4 rounded-3xl border border-outline-variant/30 bg-surface-container-lowest p-5 shadow-sm">
            ...
            <button
              ...
              disabled={loading}
              className="flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-3 font-bold text-on-primary disabled:opacity-50"
            >
          ...
          {issue && (
            <div className="mt-4 flex flex-wrap items-end gap-3 border-t border-outline-variant/20 pt-4">
              <div className="flex-1 min-w-[260px]">
                ...
                <input
                  ...
                  placeholder="https://yoursite.com/brief/..."
                  className="mt-2 w-full rounded-xl border border-outline-variant/30 px-3 py-2 text-on-surface bg-surface-container-low text-sm"
                />
              </div>
              <button
                onClick={handleSaveUrl}
                disabled={loading || urlInput === websiteUrl}
                className="flex items-center gap-2 rounded-xl bg-secondary px-4 py-3 font-bold text-on-secondary text-sm disabled:opacity-40"
              >
          ...
          {emailPanel && issue && (
            <div className="mt-4 border-t border-outline-variant/20 pt-4">
              <span className="text-xs font-bold uppercase tracking-wide text-on-surface-variant">Send Email To</span>
              <p className="text-xs text-on-surface-variant mt-1 mb-2">Separate multiple addresses with commas or spaces.</p>
              <div className="flex flex-wrap gap-3">
                <input
                  ...
                  className="flex-1 min-w-[260px] rounded-xl border border-outline-variant/30 px-3 py-2 text-on-surface bg-surface-container-low text-sm"
                />
                <button
                  onClick={handleSendEmail}
                  disabled={emailSending || !emailRecipients.trim()}
                  className="flex items-center gap-2 rounded-xl bg-primary px-4 py-3 font-bold text-on-primary text-sm disabled:opacity-40"
                >
            ...
            <div className="mb-6 flex flex-wrap items-center gap-3 rounded-3xl border border-outline-variant/30 bg-surface-container-lowest p-4 shadow-sm">
              ...
              {issue.status === "draft" && (
                <button
                  onClick={() => {
                    const sopSection = sections.find((s) => s.section_key === "sop_spotlight")
                    if (!editingSopSpotlight) {
                      setSopSpotlightTitle(sopSection?.content?.sop_title ?? "")
                      setSopSpotlightBody(sopSection?.markdown_body ?? "")
                    }
                    setEditingSopSpotlight(!editingSopSpotlight)
                  }}
                  className="flex items-center gap-2 rounded-xl border border-outline-variant/30 px-4 py-2 text-sm font-bold text-on-surface"
                >
                  <Pencil className="w-4 h-4" />
                  {editingSopSpotlight ? "Done Editing" : "Edit SOP Spotlight"}
                </button>
              )}
          ...
          <OperationsBriefPreview
            ...
            editingSopSpotlight={editingSopSpotlight}
            sopSpotlightTitle={sopSpotlightTitle}
            sopSpotlightBody={sopSpotlightBody}
            onSopSpotlightTitleChange={setSopSpotlightTitle}
            onSopSpotlightBodyChange={setSopSpotlightBody}
            onSaveSopSpotlight={handleSaveSopSpotlight}
            canEditSections={profile?.role === "manager"}
            onSaveSectionMarkdown={async (sectionId, markdown) => {
              try {
                const { updateSectionContent } = await import("@/lib/actions/operations-brief")
                await updateSectionContent(sectionId, markdown)
                setSections((prev) =>
                  prev.map((s) => (s.id === sectionId ? { ...s, markdown_body: markdown } : s)),
                )
                setMessage("Section text updated.")
              } catch (err: any) {
                setError(err?.message ?? "Unable to save section text.")
              }
            }}
          />
        </div>

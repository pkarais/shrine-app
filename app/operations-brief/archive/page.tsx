...
import { Archive, Download, ExternalLink, Eye, Loader2, Newspaper } from "lucide-react"
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      {issue.slug && (
                        <Link href={`/brief/${issue.slug}`} className="inline-flex items-center gap-2 rounded-xl bg-primary px-3 py-2 text-sm font-bold text-white">
                          <Eye className="w-4 h-4" /> View Brief
                        </Link>
                      )}
                      {issue.website_url && (
                        <a href={issue.website_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl bg-surface-container border border-outline-variant/40 px-3 py-2 text-sm font-bold text-on-surface">

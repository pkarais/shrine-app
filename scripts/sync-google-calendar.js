...
async function upsertEvent(event, existingByGid) {
  const { google_event_id, ...eventData } = event;

  if (google_event_id) {
    const existing = existingByGid.get(google_event_id);
    if (existing && eventsEqual(existing, event)) {
      // Nothing changed — skip the PATCH entirely. Saves a disk write,
      // a WAL entry, and a realtime broadcast on the events table.
      return { success: true, action: 'unchanged', status: 200 };
    }
...
async function syncCalendar() {
  ...
  // Fetch existing rows for this window ONCE so we can diff and skip
  // unchanged events instead of issuing pointless UPDATEs every dev restart.
  const selectCols = ['google_event_id', ...DIFF_FIELDS].join(',');
  const existingResp = await getFromSupabase(
    `/rest/v1/events?google_event_id=not.is.null&start_time=gte.${encodeURIComponent(timeMin)}&start_time=lte.${encodeURIComponent(timeMax)}&select=${selectCols}`
  );
  const existingByGid = new Map();
  if (Array.isArray(existingResp.data)) {
    for (const row of existingResp.data) {
      if (row && row.google_event_id) existingByGid.set(row.google_event_id, row);
    }
  }
  console.log(`Loaded ${existingByGid.size} existing events for diff`);
  ...
  for (const item of items) {
    ...
    const result = await upsertEvent(event, existingByGid);

    if (result.success) {
      if (result.action === 'unchanged') {
        unchanged++;
        // No log line — keep output quiet for the common case.
        continue;
      }
  ...
  console.log(`\nDone: ${synced} new, ${updated} updated, ${unchanged} unchanged, ${errors} errors, ${skipped} skipped`);
